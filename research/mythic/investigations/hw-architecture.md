...
## Investigation Log — 2026-08-08 Roadmap Scan

*Investigator pass appended 2026-08-08. Scope: what changed for Mythic hardware since the 2026-04-05 baseline. Verdict: **PARTIALLY_CONFIRMED** — two corporate events confirmed, zero silicon disclosed.*

### Method

1. Read Mythic's own news index (`mythic.ai/whats-new/`) end to end for the Dec 2025 → Aug 2026 span. It carries **exactly three items**: the $125 M round (2025-12-17), Honda (2026-02-06), Videantis (2026-05-19). Nothing after 2026-05-19.
2. Pulled the full acquisition press-release text from `videantis.com/mythic-acquires-videantis.html` (Videantis hosts the release verbatim; site banner reads "videantis is now part of Mythic").
3. Attempted independent corroboration. Found that edge-ai-vision, pulse2, TMCnet, the AFP wire feed, and hw.dev all republish the **same BusinessWire copy**. The only source independent of both parties is the **Taylor Wessing** advisory notice (2026-06-02) confirming the deal closed after regulatory clearance — that page returns **HTTP 403** to direct fetch and was read only via its search-result title/snippet.
4. Went to the primary for Honda: `global.honda/en/topics/2026/c_2026-02-04eng.html`. Confirmed Honda's release contains **no TOPS figure, no efficiency multiple, and no timeline** — those come solely from Mythic's 2026-02-06 copy.
5. Checked the Hot Chips 38 advance program (Aug 23–25, 2026, Stanford). **No Mythic entry.**
6. Searched for any substantiation of the repo's flagged "750× tokens/s/W on 1T models" claim. **None found.**

### Findings that changed repo content

| Finding | Disposition |
|---|---|
| Videantis GmbH acquisition, announced 2026-05-19, **closed** | New section in `hw-architecture.md`; new rows in Product SKUs, Memory Hierarchy, Interconnect; `update_2026_08_08` fact groups in the YAML |
| v-MP6000UDX = VLIW+SIMD unified processor IP; **no node, core count, throughput, or data types disclosed** | Recorded as an IP block with every quantitative field `not_disclosed` |
| Honda joint development + APU licensing (Honda 2026-02-04 primary, Mythic 2026-02-06) | Backfill — **predates the 2026-04-05 baseline**, flagged as a missed gap rather than new news |
| Mythic marketing targets: 100× efficiency, **100,000+ TOPS**, prototype vehicle testing late 2020s/early 2030s | Recorded as automotive marketing targets, explicitly not measured and not datacenter |
| "Analog Processing Unit (APU)" and "Starlight" product names | Added; both previously absent from the repo |
| eCAPITAL joined the cap table via the acquisition | Added as an equity change, explicitly **not** a priced round |
| HQ conflict: Honda says "Texas, U.S.-based"; Mythic's May 2026 dateline says "Palo Alto, CA"; repo says Austin, TX | Recorded as unresolved / dual-site |

### Pre-existing repo errors corrected

- **"Advanced node (est.)"** for the next-gen part → **not disclosed**. No primary source publishes a node for any forthcoming Mythic part.
- **"Next-gen chip targets advanced node"** in the Analog Compute Limitations section → same correction.
- **"PCIe Gen4/5 for multi-card server setups (2026+ roadmap)"** in `layer-table.md` → removed; no primary source states a host-interface generation for any forthcoming part.
- **Pure-analog characterization** → now labelled incomplete; forward architecture is analog CIM + a licensed programmable digital core.

### Deliberate non-changes

- **`hw-architecture.dot` untouched.** It depicts shipping M1076/M1108 silicon. A hybrid analog+digital part has no disclosed engine, memory tier, or fabric; drawing an announced intention as an architecture block would misrepresent the evidence.
- **The "750× tokens/s/W" claim was not deleted and was not promoted.** It stays in place, flagged unverified, with a re-check note dated 2026-08-08.
- **The programming-model line was hedged, not rewritten.** Mythic has published nothing about a merged SDK, ISA, or compiler.

### Open items for the next scan

1. Any Mythic news after **2026-05-19** — the index has been static since.
2. First disclosure of a **hybrid analog+digital part**: name, node, engine counts, memory hierarchy, fabric.
3. Any **merged SDK/ISA/compiler** announcement folding v-MP6000UDX into MAPP.
4. Whether Mythic's **datacenter** intent ever attaches to a named product, or remains marketing framing inside automotive/edge releases.
5. **Deal value, headcount, and roadmap detail** for the Videantis acquisition — none published; watch for a filing or trade-press follow-up.
6. Independent (non-BusinessWire-derived) reporting on the acquisition. Retry the Taylor Wessing page from an environment that is not 403'd.
7. Re-check the **HQ** question (Austin vs. Palo Alto vs. "Texas-based") against a filing.
